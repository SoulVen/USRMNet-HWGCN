...
from network import GraphConvolution1, GraphConvolution2, REGNN
from utils import *
from network import FCL, FCL2
import numpy as np
...
class UnfoldPOCS(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        self.K0           =    [1, 1]
        self.REGNNs       =    nn.ModuleList([REGNN(self.K0, self.F1, self.relu), REGNN(self.K0, self.F2, self.relu)]).to(self.device)
...
class uRLLCunfoldingNet(nn.Module):
    ...
    def forward(self, data_H, w0, x0_tmp, nu3_val, tilde_gamma_val, A9, A1, A3, A7, b7, A10, outer_itr, inner_itr):
        ...
        if outer_itr > 1:
            with t.no_grad():
                for j in range(outer_itr-1):
                    ...
                    # x = x_tmp.detach()
                    x = x_tmp
                    w0, obj_outer = self.update_W(x, data_H)
                    obj_USRMax[j].extend(obj_outer)
        ...
        with t.no_grad():
           w0, obj_outer = self.update_W(x, data_H)
        f_list_out = f_list
        obj_USRMax[0].extend(obj_outer)

        return x, w0, f_list_out, obj_USRMax
    ...
    def update_W(self, x_k_half, data_H):
        ...
        W_star_tau = t.zeros((self.args.batchsize, self.args.Nt, self.args.K), dtype=t.complex64)   # 存储当前的波束赋形向量
        obj_outer = []
        for loop in range(self.args.batchsize):
            ...
            hh = t.zeros(self.args.Nt, self.args.Nt)
            ...
            keshi = 0  # Initialize objective value (7a)
            for i in range(self.args.K):
        ...
        print("obj_outer: ", np.mean(obj_outer))
        return  W_star_tau, obj_outer